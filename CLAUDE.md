...
crates/ananke-shard/   The range layer (Phase 3): the node's wire — range-tagged batch frames, the per-peer outbox, the node's inbox; the node's tasks and Q41's round (round.rs, node.rs, variant.rs)
