...
are done and tagged. Phase 3, sharding, has begun: the node's wire is on `main`, and nothing
above it is. Transactions and SQL are design only. One thing the tags do not mean: `v0.3.0`
is a tag in this repository and **is not on crates.io yet**, where the newest version of every
crate is 0.2.0 and `ananke-raft` has never been published. The tables below say exactly where
things stand.
...
|---|---|---|
| `ananke-env` | Released. The `Environment` trait (`Clock`, `FileSystem` with explicit `sync` and `sync_dir`, a message-oriented `Network`, `Rng`, `spawn`); `RealEnv` on tokio; `Sim` / `SimEnv` with the §1.3 disk and §1.4 network fault models, per-node clock skew and drift, and a poll budget; the moirae format v2 export. New in 0.3.0: message duplication, the Raft trace events, and a record's decision time beside its durability time (D-047) | 0.1.0 and 0.2.0 on crates.io; 0.3.0 is tagged `v0.3.0` here and not published yet |
| `ananke-storage` | Released. The WAL, memtable, SSTables under a manifest, log truncation, versions, snapshots, scans, leveled compaction, write batches, unsynced writes and checkpoints (D-018 to D-024). New in 0.3.0: the WAL record header carries its own checksum, a format change (D-027), and a refused engine does no work (D-044) | 0.2.0 on crates.io; 0.3.0 is tagged `v0.3.0` here and not published yet |
| `ananke-raft` | Done and tagged, never published. A pure protocol core with pre-vote, read-index and lease reads with a drift guard, check quorum, leadership transfer, joint-consensus membership changes with learners, and snapshots streamed as resumable chunks of an engine checkpoint; its state in the storage engine; the server as `raft`, `net`, `apply` and `snapshot` tasks; a single-shard key-value store; lost-state refusal and re-seeding (D-025 to D-049). It runs under the simulator; no binary runs it on real sockets yet | 0.3.0, tagged `v0.3.0`. **Not on crates.io**: this crate has never been published, so `cargo add ananke-raft` finds nothing |
...
| 3 | Multi-raft sharding (§4) | Linearizability across split, merge and rebalance under faults; a 1000-range cluster stays balanced within 10% after node add and remove | Begun, nowhere near its exit criteria. On `main`: the node's wire only (the batch frame, the per-peer outbox, the inbox), under a decision still marked PROPOSED (D-072). The node's tasks, follower compaction and the snapshot task are open pull requests; descriptors, split, merge and the rebalancer are not built |
