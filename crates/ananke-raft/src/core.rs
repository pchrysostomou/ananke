...
impl Raft {
    ...
    pub fn restore(
        ...
        Self::restore_compacted(id, membership, config, seed, term, vote, 0, 0, log, false)
    }
    ...
    /// at `snap_index + 1`. With `quarantined` the server runs on a re-seeded store
    /// and grants no vote, no pre-vote and no lease promise for good (RAFT.md §3).
    #[must_use]
    ...
    fn on_timeout_now(&mut self, from: ServerId) {
        if self.quarantined || self.role == Role::Leader || self.leader != Some(from) {
            return;
        }
        self.transfer = true;
    ...
    fn maybe_compact(&mut self) {
        ...
        self.log.drain(..(index - self.snap_index) as usize);
